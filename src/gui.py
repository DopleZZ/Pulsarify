...
from data_reader import read_z_values
...
class PulsarifyGUI(QtWidgets.QWidget):
    ...
    def generate(self):
        ...
        font_dir = os.path.join('data', '11zon_zip')
        fmt = self.format_combo.currentText()
        out_basename = self.outpath_edit.text().strip() or 'output'
        try:
            if fmt == 'svg':
                out_path = out_basename if out_basename.lower().endswith('.svg') else out_basename + '.svg'
                generate_svg(groups, font_dir, out_path)
            else:
                image = generate_image(groups, font_dir)
                out_path = out_basename if out_basename.lower().endswith('.png') else out_basename + '.png'
                image.save(out_path)
