import os
from data_reader import read_z_values, read_text, get_data_dir
...
def main():
    ...
    from data_reader import get_output_dir
    out_dir = get_output_dir()
    if out_format == 'svg':
        generate_svg(groups, font_dir, os.path.join(out_dir, 'output.svg'))
    else:
        image = generate_image(groups, font_dir)
        image.save(os.path.join(out_dir, 'output.png'))
